...
<Tabs groupId="sdk-type">
  ...
  <TabItem value="ts-kit" label="TypeScript Kit">
    ...
    const { poolAddress, instructions, initializationCost, callback: createSplashPool } = await createSplashPoolInstructions(
        devnetRpc,
        tokenMintOne,
        tokenMintTwo,
        initialPrice,
    ...
    const txId = await createSplashPool();
    ...
    const { poolAddress, instructions, initializationCost, callback: createConcentratedLiquidityPool } = await createConcentratedLiquidityPoolInstructions(
        devnetRpc,
        tokenMintOne,
        tokenMintTwo,
        tickSpacing,
    ...
    // Use the callback to submit the transaction
    const txId = await createConcentratedLiquidityPool();
