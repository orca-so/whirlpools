...
export async function checkTickArrayInitialization(
  ...
  const whirlpool = (await ctx.fetcher.getPool(
    whirlpoolPubkey,
  )) as WhirlpoolData; // tickSpacing is immutable
  ...
  for (const targetState of positionBundleTargetState) {
    if (targetState.state === "open") {
      tickArrayStartIndexes.add(
        TickUtil.getStartTickIndex(targetState.lowerTickIndex, tickSpacing),
      );
      tickArrayStartIndexes.add(
        TickUtil.getStartTickIndex(targetState.upperTickIndex, tickSpacing),
      );
    }
  }
  ...
  const uninitialized = await TickArrayUtil.getUninitializedArraysString(
    tickArrayAddresses,
    ctx.fetcher,
    IGNORE_CACHE,
  );
...
export async function checkATAInitialization(
  ctx: WhirlpoolContext,
  ...
  const uninitialized = mintAddresses.filter(
    (_, i) => !atas.get(ataAddresses[i].toBase58()),
  );

  if (uninitialized.length > 0) {
    throw new Error(
      `uninitialized ATAs for mint: ${uninitialized.map((mint) => mint.toBase58()).join(", ")}`,
    );
  }
}
