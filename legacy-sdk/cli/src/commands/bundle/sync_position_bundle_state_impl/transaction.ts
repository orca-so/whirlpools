...
export async function buildTransactions(
  ...
  const ataPositionBundle = getAssociatedTokenAddressSync(
    difference.positionBundle.positionBundleMint,
    ctx.wallet.publicKey,
    true,
    TOKEN_PROGRAM_ID,
  );
  ...
  const rewardParams = await Promise.all(
    whirlpool.rewardInfos
      .filter((rewardInfo) => PoolUtil.isRewardInitialized(rewardInfo))
      .map(async (rewardInfo) => {
  ...
  const getBundledPositionPDA = (bundleIndex: number) => {
    return PDAUtil.getBundledPosition(
    ...
    withdrawTransactions.push(
      toTx(
        ...
        WhirlpoolIx.decreaseLiquidityV2Ix(ctx.program, {
          ...
          tickArrayLower: getTickArrayPubkey(
            difference.bundledPositions[bundleIndex]!.tickLowerIndex,
          ),
          tickArrayUpper: getTickArrayPubkey(
            difference.bundledPositions[bundleIndex]!.tickUpperIndex,
          ),
        }),
      ),
    );
  ...
  const mergedWithdrawTransactions = mergeTransactionBuilders(
    ctx,
    withdrawTransactions,
    alts,
  );
  const mergedDepositTransactions = mergeTransactionBuilders(
    ctx,
    depositTransactions,
    alts,
  );
