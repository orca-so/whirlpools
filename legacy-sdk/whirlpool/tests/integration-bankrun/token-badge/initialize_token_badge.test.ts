/**
 * Token Badge Initialize Test - Bankrun Version
 *
...
describe("initialize_token_badge (bankrun)", () => {
  ...
  it("successfully initialize when funder is different than account paying for transaction fee", async () => {
    ...
    assert.ok(
      tokenBadgeData!.whirlpoolsConfig.equals(whirlpoolsConfigKeypair.publicKey)
    );
    assert.ok(tokenBadgeData!.tokenMint.equals(mint));
  });
