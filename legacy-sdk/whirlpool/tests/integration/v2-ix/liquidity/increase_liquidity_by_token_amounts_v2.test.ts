      ...
      describe(`tokenTraitA: ${
        ...
        it("increase liquidity of a position spanning two tick arrays", async () => {
          ...
          await toTx(
            ...
            WhirlpoolIx.increaseLiquidityByTokenAmountsV2Ix(ctx.program, {
              ...
              minSqrtPrice: ZERO_BN,
              maxSqrtPrice: MAX_U128,
        ...
        it("fails when token max a exceeded", async () => {
          ...
          await assert.rejects(
            ...
            /0x177c/, // LiquidityZero (by-token-amounts chooses max liquidity within caps; with these params liquidity is zero)
          );
        });
  ...
  describe("v2 specific accounts", () => {
    ...
    it("fails when passed token_program_a is token_metadata", async () => {
      ...
      await assert.rejects(
        ...
        /0xbc0/, // InvalidProgramId (iterator rejects non-Token program)
      );
    });
    ...
    describe("min/max sqrt price constraints", () => {
      it("succeeds when current sqrt price is within [min, max]", async () => {
        const currTick = 0;
        const tickLowerIndex = -1280;
        const tickUpperIndex = 1280;
        const initialSqrt = PriceMath.tickIndexToSqrtPriceX64(currTick);
        const fixture = await new WhirlpoolTestFixtureV2(ctx).init({
          tokenTraitA: { isToken2022: true },
          tokenTraitB: { isToken2022: true },
          tickSpacing: TickSpacing.Standard,
        ...
        const tokenAmount = toTokenAmount(100_000, 100_000);
        ...
        await toTx(
          ...
          WhirlpoolIx.increaseLiquidityByTokenAmountsV2Ix(ctx.program, {
            tokenMaxA: tokenAmount.tokenA,
            tokenMaxB: tokenAmount.tokenB,
            // Equal to current sqrt price should be allowed
            minSqrtPrice: initialSqrt,
            maxSqrtPrice: initialSqrt,
            whirlpool: whirlpoolPda.publicKey,
            positionAuthority: provider.wallet.publicKey,
            position: positionInitInfo.publicKey,
            positionTokenAccount: positionInitInfo.tokenAccount,
      ...
      it("fails when minSqrtPrice is greater than current sqrt price", async () => {
        const currTick = 0;
        const tickLowerIndex = -1280;
        const tickUpperIndex = 1280;
        const currentSqrt = PriceMath.tickIndexToSqrtPriceX64(currTick);
        const minSqrtTooHigh = currentSqrt.add(new BN(1));
        const fixture = await new WhirlpoolTestFixtureV2(ctx).init({
          tokenTraitA: { isToken2022: true },
          tokenTraitB: { isToken2022: true },
          tickSpacing: TickSpacing.Standard,
        ...
        await assert.rejects(
          ...
          /0x17b5/, // PriceSlippageOutOfBounds
        );
      });

      it("fails when maxSqrtPrice is less than current sqrt price", async () => {
        const currTick = 0;
        const tickLowerIndex = -1280;
        const tickUpperIndex = 1280;
        const currentSqrt = PriceMath.tickIndexToSqrtPriceX64(currTick);
        const maxSqrtTooLow = currentSqrt.sub(new BN(1));
        const fixture = await new WhirlpoolTestFixtureV2(ctx).init({
          tokenTraitA: { isToken2022: true },
          tokenTraitB: { isToken2022: true },
          tickSpacing: TickSpacing.Standard,
