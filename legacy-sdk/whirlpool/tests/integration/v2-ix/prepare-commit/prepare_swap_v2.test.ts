...
describe("prepare_swap_v2", () => {
  ...
  describe("invalid accounts (halt the entire transaction)", () => {
    ...
    it("fails when token authority is not signer", async () => {
      ...
      const tokenAuthority = PublicKey.unique(); // other wallet address
      const ix = WhirlpoolIx.prepareSwapV2Ix(ctx.program, {
      ...
      assert.ok(ix.keys[1].pubkey.equals(tokenAuthority));
    ...
    // Note: Even if all provided tick arrays are invalid, the instruction should
    // return QuoteError rather than halt the entire transaction.
      ...
      await assert.rejects(
        toTx(
          ...
          WhirlpoolIx.prepareSwapV2Ix(ctx.program, {
            ...
            tickArray1: tickArrays[0].publicKey,
            tickArray2: tickArrays[0].publicKey,
            oracle: PublicKey.unique(), // invalid
          }),
        ).buildAndExecute(),
