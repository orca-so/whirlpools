...
describe("whirlpool-client-impl", () => {
  ...
  describe("TokenProgram", () => {
    ...
    it("successfully creates a new splash pool whirlpool account and initial tick array account", async () => {
      ...
      const [startTick, endTick] = TickUtil.getFullRangeTickIndex(SPLASH_POOL_TICK_SPACING);
      ...
      const [whirlpoolAccountBefore, startTickArrayAccountBefore, endTickArrayAccountBefore] = await Promise.all([
        ctx.fetcher.getPool(expectedPda.publicKey, IGNORE_CACHE),
        ctx.fetcher.getTickArray(startTickArrayPda.publicKey, IGNORE_CACHE),
      ...
      const [whirlpoolAccountAfter, startTickArrayAccountAfter, endTickArrayAccountAfter] = await Promise.all([
        ctx.fetcher.getPool(expectedPda.publicKey, IGNORE_CACHE),
        ctx.fetcher.getTickArray(startTickArrayPda.publicKey, IGNORE_CACHE),
        ctx.fetcher.getTickArray(endTickArrayPda.publicKey, IGNORE_CACHE),
      ]);
      ...
      const startSqrtPrice = PriceMath.priceToSqrtPriceX64(
        PriceMath.sqrtPriceX64ToPrice(poolInitInfo.initSqrtPrice, 6, 6), 6, 6
      )
      ...
      assert.ok(whirlpoolAccountAfter.tickCurrentIndex === PriceMath.sqrtPriceX64ToTickIndex(startSqrtPrice));
      ...
      assert.ok(startTickArrayAccountAfter.whirlpool.equals(expectedPda.publicKey));
      ...
      assert.ok(endTickArrayAccountAfter.whirlpool.equals(expectedPda.publicKey));
    });
