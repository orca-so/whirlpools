...
pub fn handler(accounts: &[AccountInfo], data: &[u8]) -> Result<()> {
    ...
    let data = crate::instruction::IncreaseLiquidityV2::try_from_slice(&data[8..])?;
    ...
    if transfer_fee_included_delta_a.amount > data.token_max_a {
        return Err(WhirlpoolErrorCode::TokenMaxExceeded.into());
    }
    if transfer_fee_included_delta_b.amount > data.token_max_b {
        return Err(WhirlpoolErrorCode::TokenMaxExceeded.into());
    }
