...
/// bundled together to keep them consistent. Pass a `TargetProgram` (or `None` to fall back
/// to [`TargetProgram::default`], which is the mutable mainnet program) to the SDK functions
/// and PDA helpers that accept it.
///
/// Use the named constructors ([`TargetProgram::mainnet`], [`TargetProgram::devnet`],
/// [`TargetProgram::mainnet_immutable`]) for the official deployments, or
/// [`TargetProgram::custom`] to point at a fork or local deployment.
...
impl Default for TargetProgram {
    /// Defaults to the mutable mainnet deployment ([`TargetProgram::mainnet`]).
    fn default() -> Self {
        Self::mainnet()
    }
}
