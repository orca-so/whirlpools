...
pub fn get_prepared_swap_address(
    ...
    let seeds = &[b"prepared_swap".as_ref(), &nonce.to_le_bytes()];
...
mod tests {
    ...
    fn test_get_prepared_swap_address_mutable() {
        let prepared_swap =
            Pubkey::from_str("5nBgXtA9LneVzwF6ctH3bMP3iefmMGLETaVbctY7ZkHw").unwrap();
        let (address, _) =
            get_prepared_swap_address(0, Some(WhirlpoolDeployment::mainnet())).unwrap();
