...
use crate::{
    ...
    open_full_range_position_instructions, open_position_instructions, swap_instructions,
...
impl TestContext {
    ...
    pub async fn open_full_range_position(&self, pool: Pubkey) -> Result<Pubkey, Box<dyn Error>> {
    ...
    pub async fn open_concentrated_liquidity_position(&self, pool: Pubkey) -> Result<Pubkey, Box<dyn Error>> {
        ...
        let position = open_position_instructions(
            &self.ctx.rpc,
            pool,
            0.9,
            1.1,
...
async fn test_splash_pool() {
    ...
    let position_mint = ctx.open_full_range_position(pool).await.unwrap();
