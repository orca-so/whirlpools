...
import { generateDefaultInitTickArrayParams, generateDefaultOpenPositionParams } from "../../utils/test-builders";
...
describe("increase_liquidity_v2", () => {
  ...
  it("increase liquidity of a position with an approved position authority delegate", async () => {
    ...
    await approveTokenForPosition(provider, positionInitInfo.tokenAccount, delegate.publicKey, 1);
  ...
  it("fails when position authority is not authorized for exactly 1 token", async () => {
    ...
    await approveTokenForPosition(provider, positionInitInfo.tokenAccount, delegate.publicKey, 0);
    await approveTokenV2(provider, tokenTraits.tokenTraitA, tokenAccountA, delegate.publicKey, 1_000_000);
    await approveTokenV2(provider, tokenTraits.tokenTraitB, tokenAccountB, delegate.publicKey, 1_000_000);
