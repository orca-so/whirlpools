...
describe("set_token_badge_authority", () => {
  ...
  async function setTokenBadgeAuthority(config: PublicKey, authority: Keypair, newAuthority: PublicKey) {
    const whirlpoolsConfigExtension = PDAUtil.getConfigExtension(ctx.program.programId, config).publicKey;
  ...
  it("successfully set token badge authority and verify updated account contents", async () => {
    ...
    assert.ok(extensionData!.whirlpoolsConfig.equals(whirlpoolsConfigKeypair.publicKey));
    ...
    assert.ok(updatedExtensionData!.whirlpoolsConfig.equals(whirlpoolsConfigKeypair.publicKey));
