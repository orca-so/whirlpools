...
import { TEST_TOKEN_2022_PROGRAM_ID, TEST_TRANSFER_HOOK_PROGRAM_ID } from "../test-consts";
import { ASSOCIATED_TOKEN_PROGRAM_ID, createUpdateTransferHookInstruction } from "@solana/spl-token";
...
export async function updateTransferHookProgram(
  ...
  return provider.sendAndConfirm(tx, !!authority ? [authority] : [], { commitment: "confirmed" });
}
...
export function createInitializeExtraAccountMetaListInstruction(
  ...
  return {
      ...
      keys: [
        ...
        {pubkey: TEST_TOKEN_2022_PROGRAM_ID, isSigner: false, isWritable: false},
